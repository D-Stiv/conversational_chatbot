...
class RegistrationForm(Form):
    ...
    def fillForm(self, state):
        try:
            ...
            form_desc = state.get_form_description()
            if form_desc is None:
                desc_intro = "it does not have a description."
            else:
                desc_intro = "{}.".format(form_desc)
            # we format the string to retrieve
            first_string = state.manage_next_step()
            string = (f"{form_intro} {desc_intro} this form contains the following fields {state.get_fields_list()}, from which {state.num_required_fields} are required" +
    ...
    def fillGenericCamp(self, state):
        try:
            ...
            slot_name_list, slot_value_list = state.set_spelling_list(
            ...
            if len(slot_value_list) + len(slot_name_list) >= 1:
                ...
                if len(slot_value_list) == 1 and len(slot_name_list) == 0:
                    ...
                    intro = state.filling_procedure(slot_name, slot_value)
                else:
                    # we have a list of fields with their values
                    intro = state.fill_generic_slots(slot_name_list=slot_name_list,
    ...
    def fillSpellingCamp(self, state):
        try:
            ...
            # we enable the close prompt because we are about to ask spelling
            state.set_close_prompt_enabled()
    ...
    def verifyPresenceOfLabel(self, state):
        try:
            ...
            for name in slot_name_list:
                text, _ = fn.verify_presence(name, state.form_slots())
                string = string + text
            return string
        except:
            if not state.get_warning_present():
        ...
        try:
            ...
            if count == 0:
                ...
                if slot_name is None:
                    string = state.manage_next_step()
                    return string
                string = state.get_field_description(slot_name)
                return string
            slot_name_list, _ = fn.extract_fields_names_and_values(
                entities)
            # we first verify if each slot_name corresponds to a field in the dorm
    ...
    def affirm(self, state):
        try:
            ...
            action_name = state.possible_next_action
        ...
        try:
            ...
            if text.lower() not in all_types and not is_number:
                string = 'Your input {} is not valid, try to insert only the character again please'.format(
                    text)
                state.set_warning_message(string)
                state.set_warning_present()
                raise Exception
    ...
    def giveRemainingOptionalLabels(self, state):
        try:
            ...
            optional_fields = state.get_string_from_list(
                remaining_optional_list)
        ...
        try:
            ...
            for slot in slots:
                ...
                if not first_found:     # the first label is put as requested slot
                    state.set_requested_slot(slot_name)
                    # we go to the filling procedure
                    state.filling_procedure(slot_name, slot_value)
                    first_found = True
